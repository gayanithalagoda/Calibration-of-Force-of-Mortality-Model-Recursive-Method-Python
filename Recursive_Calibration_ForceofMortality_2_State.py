...
# -*- coding: utf-8 -*-
"""
Created on Fri Sep  3 12:16:27 2021
...
z = pd.read_csv("/Users/gayanithalagoda/Desktop/masters thesis/MASTERS_RESEARCH_CODE/Research_Content/Datasets/Simulated_data/mttdata.csv")
z = np.array(z[0:905])
intrate=z
n=60
state = 2
...
no_passes = 10
alpha=[0.8, 0.9966]
#gamma = [0.0078, 0.0048, 0.0028]
nu = [1- 3*0.0105, 1- 6*0.0105]
fish_alpha = [0,0]
fish_nu=[0,0]

#alpha=[0.8639500537377028, 0.9243183433169593, 0.9622130498397818]
#gamma=[0.013864326191830154, 0.013330557944035912, 0.010946564014187017]
#nu= [0.008176066558838407, 0.005382302575122123, 0.0034218453783430424]
#alpha = [0.35,0.10,0.10]
#gamma = [0.0035,0.0045,0.0055]
#nu = [0.02,0.0105,0.0120]
...
lambda1 = np.exp(-(alphaIP*intrate[0])*intrate[1]/(nuIP**2) - 0.5*((alphaIP*intrate[0])/nuIP)**2)
Loglikelihood1= -0.5*np.log(2*math.pi*nuIP**2)- (intrate[0]-alphaIP)**2/(2*nuIP**2)
...
for u in range(0,no_passes):
    ...
    for k in range(0,batch+1):#batch+1
        ...
        for i in range(0,state):
            ...
            Gamma[k,i]=math.exp(-(IR[k+1]*(alpha[i]*IR[k])/nu[i]**2) - ((alpha[i]*IR[k])**2)/(2*nu[i]**2))
    ...
    for k in range(0,batch):
        ...
        for i in range(0,state):
            ...
            c21 = nu[i]*Xk2[i]
            nuIP2 = nuIP2 + c21
        intr[k+1] = alphaIP1*intrate[inta+k-2] 
        #intr[k+1] = alphaIP1*intrate[inta+k-2] + gammaIP1# dimension issues
        #Loglikelihood[k] = -0.5* np.log(2*math.pi*nuIP1**2)- (intrate[inta+k-1]- alphaIP1*intrate[inta+k-2] - gammaIP1)**2/(2*nuIP1**2)
        Loglikelihood[k] = -0.5* np.log(2*math.pi*nuIP1**2)- (intrate[inta+k-1]- alphaIP1*intrate[inta+k-2])**2/(2*nuIP1**2)# if remove k , it works
    ...
    for r in range(0,state):
        ...
        alpha[r]= (remainderST3[r,inta+batch-2])/remainderST2[r,inta+batch-2]
   #for r in range(0,state):
    ...
    for r in range(0,state):
        ...
        nu[r] = math.sqrt(abs(remainderST2[r,inta+batch-1]+ (alpha[r]**2) * remainderST2[r,inta+batch-2]  - 2*alpha[r]*remainderST3[r,inta+batch-2])/remainderSO[r,inta+batch-2])
    for r in range(0,state):
        fish_alpha[r]= remainderST2[r,inta+batch-2]/nu[r]**2
    for r in range(0,state):
        fish_nu[r] = (remainderSO[r,inta+batch-2]/nu[r]**2) + 3*(remainderST2[r,inta+batch-1] + (alpha[r]**2) * remainderST2[r,inta+batch-2]  - 2*alpha[r]*remainderST3[r,inta+batch-2])/nu[r]**4
